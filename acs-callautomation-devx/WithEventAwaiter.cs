...
namespace CallAutomation.DevX
{
	public static class WithEventAwaiter
	{
		public static void AddEventAwaiter(this WebApplication app)
		{
			...
			{
				var createCall = await callAutomationClient.CreateCallAsync_(
				...
				var recognize = await callMedia.StartRecognizingAsync_(new CallMediaRecognizeDtmfOptions(new PhoneNumberIdentifier(targetNumber), 10)
				{
				...
				{
					var play = await callMedia.PlayToAllAsync_(success ? new FileSource(rightAnswer) : new FileSource(wrongAnswer));
